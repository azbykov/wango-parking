...
router.post('/start-parking', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        res.status(201).json({ message: 'Parking started', session })
    } catch (err) {
        console.error(err)
...
router.post('/stop-parking', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
        const session = await ParkingSession.findOne({ user: userId, endTime: null }).populate({
            path: 'area',
            populate: { path: 'city' },
        })
